...
export class DatatableComponent implements OnInit, DoCheck, AfterViewInit {
  ...
  /**
   * Array of filters for columns.
   * Default value: `[]`
   */
  @Input() filters: any[] = [];

  /**
   * Array of colWidths for columns.
   * Default value: `[]`
   */
  @Input() colWidths: any[] = [];
  ...
  /**
   * A property holds the edit mode: true = edit cell on Click
   */
  ...
  /**
   * The filter icon of a column header was clicked.
   */
  @Output() filter: EventEmitter<any> = new EventEmitter();

  /**
   * The cell end edit value triggers.
   */
  ...
  storedSize() {
    this._internalColumns.map(x => {
      for (let y = 0; y < this.colWidths.length; y++) {
        if (this.colWidths[y].prop === x.prop) {
          x.width = this.colWidths[y].size;
        }
      }
    });
  }
  ...
  */
  ...
  onBodyScroll(event: MouseEvent): void {
    ...
    this.scrollbarHelper.setScrollXPos(this.gridId, event.offsetX);
  ...
  onColumnReorder({ column, newValue, prevValue }: any): void {
    ...
    const dispCols = this.bodyComponent.displayedColumns.map(c => {
      return { ...c };
    });
    ...
    this._internalColumns = cols;
    this._displayedColumns = dispCols;
    ...
    console.log('ngx cols', cols);
    console.log('ngx dispCols', dispCols);
    console.log('ngx column', column);
    console.log('ngx newValue', newValue);
    console.log('ngx prevValue', prevValue);
    console.log('ngx internal', this._internalColumns);
    console.log('ngx displayed', this._displayedColumns);
  }
  ...
  resetScrollXPos(gridId: string) {
    this.scrollbarHelper.clearScrollXPos(gridId);
  }
