import {
  ...
  EventEmitter, HostListener, ChangeDetectionStrategy, ChangeDetectorRef, DoCheck, SkipSelf, OnDestroy, OnInit
} from '@angular/core';
...
export class DataTableBodyRowComponent implements DoCheck {
  ...
  @Input() set innerWidth(val: number) {
    if (this._columns) {
      const colByPin = columnsByPin(this.visibleColumns);
  ...
  get visibleColumns(): any[] {
    ...
    return this.columns.filter(col => this.hiddenColumns.indexOf(col.prop) === -1);
  }
  ...
  onEdit(event: any, index: number): void {
    event.cellIndex = index;
    event.rowElement = this._element;
  ...
  recalculateColumns(val: any[] = this.columns): void {
    this._columns = val;
    const colsByPin = columnsByPin(this.visibleColumns);
    this._columnsByPin = columnsByPinArr(this.visibleColumns);
    this._columnGroupWidths = columnGroupWidths(colsByPin, this.visibleColumns);
  }
